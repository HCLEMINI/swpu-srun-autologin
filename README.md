...
WiFi 显示满格却连不上、或登录程序一直报错时，**双击 `网络诊断.bat`**（需本机装有 Python）。
它会自动收集：WiFi 关联状态与信号强度、IP/网关/DNS、ping 校园门户、ping 网关，
并给出**"是链路问题还是认证问题"**的初步判断，结果写入同目录 `网络诊断报告.txt` 并自动打开。

- 报告显示"门户可达" → 直接用本程序登门户即可。
- 报告显示"无有效 IP / 169.254.x / 无网关" → 关联或 DHCP 失败，属链路/驱动/电源问题（关网卡电源管理、更新驱动、锁频段）。
- 报告显示"网关通但门户不通" → 多半不在校园网内（如手机热点，门户是内网地址到不了），属正常。
