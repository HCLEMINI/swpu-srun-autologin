...
| ⏯ 开机自启 | **任务计划（系统启动时触发）**，以 SYSTEM 无界面服务运行，**先于桌面程序联网**（需一次 UAC 确认） |
...
3. 勾选「**开机自启（系统启动时联网）**」再保存 → 弹出一次 UAC，点【是】。
   开机后程序作为**系统服务**自动运行，**先于其它桌面程序把网连好**，其它应用一启动即可联网。
...
普通"开机自启"（注册表 `Run` 键 / 启动文件夹）要等**用户登录后**才运行，此时其它桌面程序也在抢着启动，容易出现"网还没连上，应用已经开始报错"。

本项目改用 **Windows 任务计划**，触发条件为 **"系统启动时"（boot，先于用户登录）**，以 `SYSTEM` 账户运行无界面服务（`--headless`）：

- 🚀 **真正的最早启动**：开机即联网，等你登录、其它程序启动时网络已就绪。
- 🔑 **SYSTEM 账户**：无需 Windows 登录密码；创建/删除任务需管理员权限，故会弹**一次 UAC** 确认。
- 📜 服务日志写在 EXE 同目录的 `srun_service.log`，可据此确认开机是否已联网。

> 任务由 GUI「保存设置」时自动创建/移除；也可手动用 `schtasks` 管理，任务名 `SrunAutoLogin-Boot`。
> 说明：开机自启走的是无界面服务（仅负责联网与断连重连）；需要看界面时，再双击 EXE 打开托盘程序即可。
